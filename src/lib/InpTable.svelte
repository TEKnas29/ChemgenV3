  ...
  function ansProModuleList(mq, gs) {
    ...
    if (mq.length > 0) {
      for (const m of mq) {
        if (m.editortype !== "") {
          if (m.editortype < 3) {
            const list_a = `I${m.id}:{"${editortype[m.editortype]["name"]}_I${
              m.id
            }"}`;
            list.push(list_a);
          } else {
    ...
    if (gs.length > 0) {
      for (const g of gs) {
        if (g.editortype !== "") {
          if (g.editortype < 3) {
            const list_a = `GS${g.id}:{"${editortype[g.editortype]["name"]}_GS${
              g.id
            }"}`;
            list.push(list_a);
          } else {
      ...
      <ButtonGroup size="md">
        ...
        <Label class="border-t"
          ...
          <Select
            items={editortype}
            placeholder="Editor"
            class="rounded-none w-auto"
            bind:value={g.editortype}
          />
        </Label>
